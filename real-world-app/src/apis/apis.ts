...
import {Article, ArticleListPayload, ArticlePayload, ArticleRes} from "../types/article.ts";
...
export const createApis = ({onUnauthen, token}: Props) => {
  ...
  return {
    ...
    profile: {
      getProfile: async ({username}: ProfilePayload) => fetcher.get(`/profiles/${username}`),

      followUser: async ({username}: ProfilePayload) => fetcher.post(`/profiles/${username}/follow`),

      unfollowUser: async ({username}: ProfilePayload) => fetcher.delete(`/profiles/${username}/follow`),
    },

    article: {
      getGlobalFeed: ({page, limit = ARTICLES_PER_PAGE}: ArticleListPayload) => fetcher.get(`/articles?limit=${limit}&offset=${page}`),
      ...
      getFavoritedArticles: async ({username, page, limit = ARTICLES_PER_PAGE}: ArticleListPayload) =>
        fetcher.get(`/articles?favorited=${username}&limit=${limit}&offset=${page}`),

      getSingleArticle: async ({slug}: {slug: string}) => fetcher.get(`/articles/${slug}`),
      ...
      updateArticle: ({slug, title, description, body, tagList}: ArticlePayload): Promise<ArticleRes> =>
      ...
      likeArticle: async ({slug}: ArticlePayload) => fetcher.post(`/articles/${slug}/favorite`),

      unlikeArticle: async ({slug}: ArticlePayload) => fetcher.delete(`/articles/${slug}/favorite`),

      getComments: async ({slug}: ArticlePayload) => fetcher.get(`/articles/${slug}/comments`),
      ...
      deleteComment: async ({slug, id}: ArticlePayload) => fetcher.delete(`/articles/${slug}/comments/${id}`),
    },
